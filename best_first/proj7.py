...
Preforms a best first search on the states of the 8-puzzle to solve the puzzle
uses a heuristic of counting the numebr of pieces are in the correct spot
...
class EightPuzzle:
    ...
    #returns value of how "good" a certain state is low = good, high = bad
    #checks how many tiles are in the correct position
        ...
        for i in range(3):
            for j in range(3):
                if state[i][j] == goal[i][j]:
                    cost -= 1 #if a tile is in the right spot heuristic score gets better
        return cost

    #preforms a best_first search with a depth bound to solve an 8 puzzle
    #given a starting state 
    def best_first(self):
        #def of varibles used for the search algorithm
        start = self.state_lst[0]
        open_lst = [] #implemented as a priority queue
        closed = []
        children = []
        state_level = [] #parrallel vector to open_list holds level int the graph of corresponding state in open_lst
        loop = 0
        DEPTH_BOUND = 5
        #passes tupels into open_lst and state level where 1st value is heuristic score, and second value
        #is the state or level, do this so when I sort the lsits they are in the order of a priority queue
        ...
        while(open_lst):
            ...
            if(cur_level[1] < DEPTH_BOUND):    #implementation of depth bound wont generate states bellow a certain level
                ...
                #loop to decide what to do with the new generaated states
                for new_child in new_children:
                    ...
                    closed.append(cur)
                    #sorts based on 1st element of tuple, makes list into priority queues
                    open_lst.sort()
                    state_level.sort()
...
def main():
    ...
    #preform best first search on the puzzle
    p.best_first()
